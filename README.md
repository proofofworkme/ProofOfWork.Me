...
- WORK network value has two confirmed views. Live network value is the active site and floor value, repricing historical confirmed WORK movement at the current live floor. Frozen network value is the immutable confirmation-time audit stamp for the same events. A fresh WORK transfer, sale, or marketplace mutation uses the current live WORK floor when it confirms, then stores that amount as frozen event value.
