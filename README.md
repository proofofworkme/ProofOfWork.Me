...
- Lets any connected mainnet sender with authoritative spendable WORK attach canonical WORK credit to a message, keeps Infinity Bond WORK attachments on the V1 sender allowlist, and lets any connected mainnet holder with an authoritative confirmed WORK balance attach spendable WORK to an Inception Bond. The transaction writes the normal payment and `pwm1:` payloads first, then the WORK registry mutation payment and the era-valid atomic WORK payloads (`pwt1:send2` before the Q16 activation boundary, `pwt1:send3` afterward) in the same locally signed transaction.
...
- A generic credit mint is valid only when its confirmed definition precedes the mint in canonical block/transaction order. Generic `pwt1:create` and `pwt1:mint` events cannot create or issue reserved POWB/INCB assets; only their matching confirmed bond projections can issue them.
...
- INCB issuance validation uses the canonical Q8 integer value fields for production-scale arithmetic; floating-point display fields are compatibility metadata and cannot veto valid exact issuance. Ordered replay also carries confirmed invalid mint dispositions from the exact hashed H-1 checkpoint, so a later verifier fix cannot resurrect a bond attempt that was already canonically rejected.
- After AMO V5, the INCB verifier binds each synthetic mint to an accepted `pwm1:m:incb` parent and each declared WORK attachment to an accepted same-transaction `pwt1:send2` or `send3`. It derives one exact issuance record per bond recipient from the hash-bound green H-1 summary, retains Q16 subatoms for `send3`, and checks both the AMO opening value and raw sequencer mint total. An unmatched declared attachment or unavailable H-1 summary fails closed. AMO transition values can corroborate that H-1 value but do not replace the required green summary.
- Ordered credit replay records insufficient-balance WORK rejections per protocol output, even when another action in the same transaction succeeds. A current-block Inception attachment can be rejected without stopping the index only when that exact WORK rejection, bond, seed mint, transaction position, and block hash all agree; dependency/oracle failures and malformed persisted v2 issuance still fail closed.
- The two bond families are classified by their exact mail memo before synthetic supply is derived: `powb` can issue only POWB and `incb` can issue only INCB. POWB remains one-for-one with direct confirmed proof payments. For INCB, bond proof payments and attached canonical WORK remain separate canonical lanes in the transaction. A valid recipient-matched same-transaction WORK transfer is valued from the H-1 summary; every transaction in the bond block is excluded from that valuation.
- `confirmed_incb_issuance = floor(direct_bond_proofs + attached_work_amount * (h_minus_one_live_work_network_value / 21_000_000))`. One whole proof in that H-1 value issues one INCB; sub-proof dust remains network value. The same hash-bound H-1 floor is the immutable confirmation value of the one shared WORK transfer, so INCB issuance and WORK frozen accounting cannot disagree. Confirmation fixes the resulting INCB balance, supply, and attached WORK proof value. `incb_network_value = fixed_cumulative_issuance_value + confirmed_incb_sale_volume + confirmed_incb_transfer_fees + confirmed_incb_marketplace_mutation_fees`, and `incb_floor = incb_network_value / confirmed_incb_supply`. Current or later WORK network value never reprices INCB. The underlying WORK movement remains one shared Growth/WORK ledger event and is not added to global network value a second time.
- `/api/v1/marketplace-summary` returns a reconciled, bounded listing preview with exact active counts rather than trusting a raw proof-index snapshot. Complete Buy books hydrate from the cursor-paginated canonical listing route; stale compacted snapshots cannot hide confirmed sealed tickets from that full book. The summary route may recover a projection row where a seal txid was temporarily written as the close txid only after first-party outspend truth proves the original listing ticket remains unspent.
- Compact marketplace listing arrays are previews. AMO renders a complete book only after paging `/api/v1/token-history?kind=listings` by its opaque cursor and proving stable checkpoint, snapshot, Core outpoint, protocol membership, and count evidence against the summary. If exact hydration is unavailable, the UI labels the preview incomplete and does not claim an empty or search-complete book; market-log rows never become active inventory. Wallet-owned listing hydration uses the same complete cursor contract.
...
- WORK settings are 21,000,000 max supply, 1,000 WORK per mint, 1,000 proofs per mint, and the `work@proofofwork.me` registry address. Historical canonical state uses eight decimal places: one WORK is 100,000,000 atoms. The active V8 precision era expands divisibility from its declaration-bound activation to sixteen decimal places: one WORK is 10,000,000,000,000,000 subatoms, and every pre-activation atom converts to exactly 100,000,000 subatoms. This preserves the exact 21,000,000 WORK supply and every holder's proportional and absolute WORK balance; it does not mint supply. Exact API/read-model projections pair canonical human decimal strings with versioned integer `*Atoms` or `*Subatoms` strings and never use binary floating point for authority. The launch price is exactly 1 proof per WORK. The credit create form can reuse the same economic template for new tickers, but cannot create another WORK-like credit.
...
- Historical WORK Marketplace Pricing Protocol V2 is anchored by declaration txid `4c53252c6e9279726e1456f4d846274bfa33f778b633d32a68ed36906b38083f`. It activated in the block after that declaration confirmed. Its governed WORK list, seal, and buy actions used `pwt-sale-v3`, committed the exact hash-bound H-1 live network value in Q8, and required `priceSats >= ceil(amountAtoms * liveNetworkValueQ8 / (21,000,000 * 100,000,000 * 100,000,000))`. Missing, stale, mismatched, below-floor, or unverifiable actions failed closed without mutating canonical WORK state. This remains replayable history; pre-V6 governed WORK actions use the AMO Unit Protocol V2 authorization `pwt-sale-v5`.
- WORK AMO Unit Protocol V2 is the prior corrective declaration, txid `54d7a367a3998ce1327ee89d983a25c80ce34b96d9811807df215a8694aead36`, confirmed at height 959620/index 141 and active from height 959621 until the V6 cutover. Its governed actions used `pwt-sale-v5`; faces were exactly 2000, 5000, or 10000 USD cents. Confirmed state is ordered by `(blockHeight, blockTransactionIndex, protocolVout, recordOrdinal)`. The latest valid `pwa1:usd1` quote strictly before the listing and `Nbefore(listing)` fed integer-only floor/ceiling formulas; confirmation permanently froze the result before the listing's own bond was applied. Valid V5 listings confirmed before V6 activation remain settleable without a later floor or quote recheck; new V5 listings from the V6 activation height are invalid audit history.
- WORK AMO Unit Protocol V6 is anchored by declaration txid `975fd82aa84995e014b240618ee1a1254d0a735e6e1241372d0bed0a0d9f0799`, confirmed at height 960218/index 102 in block `00000000000000000001ac35a5b7e43c782297fcb9cde0fb458fbd5451ad55df`, with protocol activation height 960219. It uses `pwt-sale-v6`; faces are exactly 20,000, 50,000, or 100,000 proofs. For face `F` and Q8 network value `N` immediately before the listing, `unitPriceProofs=F`, `unitAmountAtoms=floor(F*21000000*100000000*100000000/N)`, and `unitMinimumPriceProofs=ceil(unitAmountAtoms*N/(21000000*100000000*100000000))`. Confirmation freezes those terms before applying the listing's bond. USD is display-only and cannot affect validity. Declaration evidence pins the exact raw `pwm1:m` carrier at vout 3, record ordinal 0, 3,350 bytes, SHA-256 `b43daeea38fcacaf6afa6a48d3d0fde631497a4af9f3bb137fc07975d18bbe01`; `pwm1:s`/`pwm1:r` mail parts may aggregate at a different mailbox position, and independently valid sibling records such as `pwt1:send2` do not invalidate that carrier. Valid V4/V5 listings confirmed before activation remain settleable without repricing. Production admission additionally requires exact declaration pins, the immutable proof-native migration marker, activation-through-tip replay parity, exact-tip health, and `WORK_AMO_V6_WRITES_ENABLED=1`; disagreement closes V6 writes.
- Historical status: V7 never acquired a confirmed declaration or activation boundary. Its pins remain empty, `WORK_AMO_V7_WRITES_ENABLED=0`, and V8 supersedes it. The complete staged V7 proposal is preserved in the next item for audit history and is not current protocol authority.
- WORK Precision Protocol V2 / AMO Unit Protocol V7 is staged but not active until its exact declaration transaction confirms. If that declaration is in block `D`, activation is `D+1`. At the opening of `D+1`, every confirmed canonical pre-activation WORK atom in balances, reservations, supply, and still-active listing projections converts to exactly 100,000,000 subatoms. Volatile pending WORK event, listing/action, and balance-delta projections are purged while noncanonical transaction envelopes remain raw recovery input; current pending projections are rebuilt from one stable Core mempool under V7 rules with exact membership, semantic, transaction, and balance parity, so an unconfirmed legacy write cannot cross the boundary. Raw confirmed record bytes, original frozen terms, and pre-activation canonical transition/snapshot commitments remain byte-for-byte and value-for-value history; provisional or wrong-era derived projections at `D+1` or later are invalidated and deterministically replayed from raw canonical evidence. New transfers then require `pwt1:send3`, and new governed listings require `pwt-sale-v7`. The raw mint record remains exactly `pwt1:mint:<canonical-work-token-id>:1000`: it credits `100000000000` Q8 atoms before activation and `10000000000000000000` Q16 subatoms from activation; every other raw WORK mint amount is invalid. For face `F`, Q8 network value `N`, `S=21000000`, `A=10000000000000000`, and `Q=100000000`, V7 serializes `unitPriceSats=F`, `unitAmountSubatoms=floor(F*S*A*Q/N)`, and `unitMinimumPriceSats=ceil(unitAmountSubatoms*N/(S*A*Q))`; those integer `*Sats` fields are denominated in proofs, while “proofs” is display language rather than an alternate field name. V6 remains authoritative and usable before activation. Once canonical `D+1` is observed or persistently latched, clearing or malforming operator pins cannot re-enable V6 or `send2`; any readiness disagreement only closes governed writes. V7 writes fail closed until exact declaration evidence, the deterministic migration marker, activation-through-tip replay parity, database constraint-definition checks, exact-tip health, and its independent write gate all agree.
- WORK Precision Protocol V2 / AMO Unit Protocol V8 is the active additive
  era. Declaration tx
  `f90e1faf572ef8253ca5959731b9d9e99c74bced4397380059878936712bee7a`
  is canonical at height 960600/index 2369 in block
  `00000000000000000001ec938998cde4fd86ee6e3c672a6d3d95200cd8a984ac`;
  activation is the opening of height 960601. At that boundary every confirmed
  current Q8 WORK amount in maximum supply, mint increment, supply, and
  balances converts by exact integer multiplication to Q16 at `100000000`
  ...
  exact terms are `unitPriceSats=F`,
  `unitAmountSubatoms=floor(F*S*A*Q/N)`, and
  `unitMinimumPriceSats=ceil(unitAmountSubatoms*N/(S*A*Q))`. Multiplication
  precedes division and confirmation freezes the result before the listing's
  bond contribution. At activation every active or sealing pre-V8 WORK listing
  is removed from the actionable book, its reservation is released, and it
  remains only as an evidence-bound relic with no legacy seal, buy, or delist
  path; V8 settlement accepts only confirmed V8 frozen terms. On 2026-08-01,
  the exact V8 pins, migration, replay, parity, and readiness evidence completed
  and gate one opened with `WORK_AMO_V8_WRITES_ENABLED=1`; Q8/V6 cannot be
  re-enabled after the boundary. V8 writes remain admitted only while the exact
  declaration carrier, block/index/output/record/
  authority/registry-payment pins, the immutable Q8-to-Q16 migration and
  relic-cutover marker, activation-through-tip replay, installed constraint
  definitions, API/worker/index/ledger parity, exact-tip Core readiness, and
  the independent V8 write gate all agree.
- The AMO V5 cutover preserves pre-unit listing attempt `4e9cedced2252cd183608dc9176415a913c4f6aa5e8307a732179a2240b6feb1` only as confirmed invalid audit history: height 959241/index 2601, protocol output 1, ordinal 0, reason `work-market-v2-canonical-oracle-unavailable`, authorization `pwt-sale-v3`, 1,600 WORK atoms, 1,500,479-proof price, and `refundEligible:false`. Its related pre-unit seal attempt remains invalid audit history too. Active listings, closed listings, and market-log views must stay terminal-empty for this listing so no reservation, relic close, seal, buy, refund, stale snapshot, or declaration-derived history can be manufactured.
...
- That immutable H-1 bootstrap contains one evidence-bound historical carry under `canonical-work-amo-v5-legacy-bootstrap-carry-v1`, from invalid WORK listing tx `5eb0a876603a7551653806b932533dc27a884631a581caa2e36dcf129b8278e8`. The exact canonical evidence is height 959311/index 2552, protocol output 1, ordinal 0, block hash `000000000000000000005a63a2c00834b92746ab0658c9f0c98aeb509724e8f9`, raw invalid-history reason `work-market-v2-canonical-oracle-unavailable`, and legacy reconciliation reason `work-market-v4-version-required`. The transaction contributes no valid marketplace activity, mutation fee, miner-fee flow, listing reservation, or derived state. Its 546-proof mutation payment and 2,216-proof miner cost survive only inside the already committed legacy-bootstrap basis. Valid outward marketplace fee/flow aliases exclude the 546 proofs and expose raw/effective/overlap reconciliation: `legacyBootstrapMarketplaceCarrySats: 546`, `legacyBootstrapSats: 2730`, `legacyBootstrapGrowthValueQ8: "273000000000"`, `legacyBootstrapRawCreditFixedSats: 2762`, `legacyBootstrapRawCreditFixedQ8: "276200000000"`, `legacyBootstrapCreditFixedSats = legacyBootstrapRawCreditFixedSats - legacyBootstrapCreditFixedOverlapSats`, and `legacyBootstrapCreditFixedQ8 = legacyBootstrapRawCreditFixedQ8 - legacyBootstrapCreditFixedOverlapQ8`, with exact `workAmoV5LegacyBootstrap` evidence. Any evidence mismatch fails closed. The committed transition N/Q8 values remain unchanged; retroactively reseeding height 959620 requires a new on-chain protocol version.
...
- Growth includes confirmed Boost observations at its exact ledger snapshot and the `2026-09-05-all-products-v1` forecast. The [current report](output/proofofwork-computer-agent-adoption-model.md), JSON, and charts share the frontend's model source. IDs, Mail, Files, Browser authoring, Boost, POWB/INCB bonds, Credit creation/minting, Wallet transfers, and AMO asset trades have explicit usage, payment, fee-elasticity, and blockspace assumptions; read-only apps and shared carriers add no duplicate value. The [May baseline archive](output/historical/2026-05-13/README.md) preserves the original report, data, and images. Forecasts remain uncalibrated scenarios from the May origin, not current chain measurements or canonical replay. Boost economic fields remain unavailable where payment or sale validation cannot be proved. The [Boost accounting proposal](BOOST_GROWTH_ACCOUNTING_PROPOSAL.md) remains unactivated; forecasts and observations do not change WORK floor, H-1 commitments, or frozen AMO terms.
...
- A Marketplace summary may publish a new surface snapshot only after exact Core reconciliation changes its active listing membership. That payload must expose `derivedFromSnapshotId` equal to the canonical ledger snapshot, keep every Marketplace component on one new snapshot id, and retain the identical indexed height and block hash. The production ledger audit accepts only that explicit one-generation derivation; an unbound, nested-mixed, height-only, or hash-divergent payload fails closed.
...
- Both the public “Post a Boost” and “What’s happening?” buttons open the same composer with Proof and WORK signal choices, including WORK-only, and Files attachments. Computer Mail compose also publishes Boost originals through the Boost ticker with its positive Proof self-send requirement and optional WORK attachment. Proof signal is the self-send amount; WORK signal uses a canonical local-wallet WORK transfer in the same transaction.
...
- `pwb1:like`, `pwb1:reply`, `pwb1:reboost`, `pwb1:follow`, and `pwb1:unfollow` are paid social actions. Likes, replies, and reboosts pay at least 546 proofs directly to the current Boost owner, and the confirmed owner-directed amount increments the original Boost’s proof signal. A follow sends 546 proofs to the followed profile, not the Boost registry; unfollow retains its profile-target payment rule. Self-follow is invalid. The shared fee-rate dashboard lets users select miner fees for the direct composer, replies, likes, and reboosts. The latest confirmed follow/unfollow event for a follower-address plus target-address pair determines the active graph edge.
...
- Confirmed ownership changes route future likes, replies, and reboosts to the new owner without retroactively rewriting prior payments. Replies and reboosts show their own paid action signal on the action card while the original Boost carries the owner-directed signal in canonical totals. A nested reboost opens its original Boost detail from the feed or expanded detail view.
...
- The Boost UI has two top-level timeline tabs: For You shows all visible confirmed Boosts, while Following shows the connected wallet’s own authored Boosts alongside visible confirmed Boosts from addresses it actively follows. Both tabs can rank confirmed posts by attached value, newest, or oldest and can scope value by hour, day, week, or all time. Total signal is the proof-equivalent value of direct proof signal plus attached WORK at the current floor; proof signal, WORK signal, and total USD value remain visible as separate lanes.
...
`check:mail-regressions` proves indexed Inbox/Sent mail, Infinity Bond Log/Event search for the OTC self-send regression tx, and subject/body rendering for historical mail whose body must be repaired from raw tx data. `check:marketplace-regressions` proves WORK delist, sale, wallet, bounded-summary counts, complete paginated sealed listing visibility, sold-listing closure, the first confirmed V6 listing's exact singleton projection and canonical position, POWB transfer visibility, and Log close/sale/transfer status stay aligned. `indexer:parity` proves the database snapshot, event rows, participants/refs, registry, summaries, token history, address-mail, and tx-status samples match the canonical ledger contract.
