...
- Treats a valid sale-ticket seal spend as active sealed inventory, not as a closure. If an indexed or cached projection has `closeTxid` equal to `sealTxid`, Marketplace recovers it as a buyable sealed listing unless a later real buy, delist, or non-seal spend closes the ticket.
...
- `/api/v1/marketplace-summary` returns the reconciled marketplace lifecycle rather than a raw proof-index summary snapshot, so stale compacted snapshots cannot hide confirmed sealed listings from the public Buy book. Proof-index summary snapshots are still backfilled and checked by parity, but route output must preserve the confirmed sealed inventory contract, including older sealed listings and projection rows where the seal txid was temporarily written as the close txid.
