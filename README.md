...
- WORK Precision Protocol V2 / AMO Unit Protocol V8 is the approved additive era. Its exact declaration transaction is selected by earliest valid canonical `(blockHeight, blockTransactionIndex)` evidence; if it confirms in block `D`, activation is the opening of `D+1`. At that boundary every confirmed current Q8 WORK amount in maximum supply, mint increment, supply, and balances converts by exact integer multiplication to Q16 at `100000000` subatoms per historical atom. One WORK is `10000000000000000` subatoms and one subatom is exactly `0.0000000000000001 WORK`. No supply or ownership changes and raw confirmed history is not rewritten. Pending WORK events, actions, listings, and balance deltas are cleared, then rebuilt from one stable mempool under active V8 semantics. From activation, current transfers require `pwt1:send3`; new governed listings require `pwt-sale-v8` and the sole `unitFaceProofs=25000`. For Q8 network value `N` immediately before the listing, `F=25000`, `S=21000000`, `A=10000000000000000`, and `Q=100000000`, exact terms are `unitPriceSats=F`, `unitAmountSubatoms=floor(F*S*A*Q/N)`, and `unitMinimumPriceSats=ceil(unitAmountSubatoms*N/(S*A*Q))`. Multiplication precedes division and confirmation freezes the result before the listing's bond contribution. At activation every active or sealing pre-V8 WORK listing is removed from the actionable book, its reservation is released, and it remains only as an evidence-bound relic with no legacy seal, buy, or delist path; V8 settlement accepts only confirmed V8 frozen terms. The additive preactivation release ships with every V8 declaration pin empty and `WORK_AMO_V8_WRITES_ENABLED=0`, leaving Q8/V6 authoritative. After activation, V6/send2 cannot be re-enabled. V8 writes open only when exact declaration carrier, block/index/output/record/authority/registry-payment pins, the immutable Q8-to-Q16 migration and relic-cutover marker, activation-through-tip replay, installed constraint definitions, API/worker/index/ledger parity, exact-tip Core readiness, and the independent V8 write gate all agree.
