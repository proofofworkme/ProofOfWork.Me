...
- The two bond families are classified by their exact mail memo before synthetic supply is derived: `powb` can mint only POWB and `incb` can mint only INCB. Bond proof payments and attached canonical WORK are separate canonical lanes in the same transaction. Attached credit is WORK-only, uses the normal `pwt1:send` mutation, and must never be mistaken for POWB or INCB mint supply.
- INCB floor accounting is confirmed Inception Bond composite network value divided by confirmed INCB supply. The summary joins a valid confirmed same-transaction WORK attachment to its Inception bond: frozen value uses the WORK floor at confirmation, while live value reprices the attached amount at the current WORK floor. INCB supply remains the confirmed bond proofs only. Inception proof payments, INCB seller sale volume, INCB transfer fees, and INCB marketplace mutation fees continue through their bond/marketplace lanes; the underlying WORK movement remains one shared Growth/WORK ledger event and is not added to global network value a second time.
