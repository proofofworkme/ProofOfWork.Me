...
- `pwb1:like`, `pwb1:reply`, `pwb1:reboost`, `pwb1:follow`, and `pwb1:unfollow` are paid social actions. Likes, replies, and reboosts pay at least 546 proofs directly to the current Boost owner, and the confirmed owner-directed amount increments the original Boost’s proof signal. Follow/unfollow payments go to the addressed profile target; social actions do not pay the Boost registry. The shared fee-rate dashboard lets users select miner fees for the direct composer, replies, likes, and reboosts. The latest confirmed follow/unfollow event for a follower-address plus target-address pair determines the active graph edge.
