...
if [[ ! "${mode}" =~ ^(--plan|--apply)$ || ! "${run_id}" =~ ^[0-9]{8}T[0-9]{6}Z$ || $# -gt 2 ]]; then
  echo 'Usage: proofofwork-audit5-restore-check.sh --plan|--apply YYYYMMDDTHHMMSSZ' >&2
  exit 64
fi
backup=/data/proofofwork-postgres-backups/logical/proof_indexer-20260905T031855Z.dumpset
job="/data/proofofwork-audit5-restore-${run_id}"
...
[[ "$(stat -c %s "${backup}/proof_indexer.dump")" == 11074733418 ]]
expected_checksums=$'77bdb885f6dd86cf4f203aebed6a590476d307ffa0c3c217ce71234aee6a9760  proof_indexer.dump\nade1dc0d764b64749b214429b2d962ed8124a42331e89d4f52e1c3821d49d3c5  globals.sql'
