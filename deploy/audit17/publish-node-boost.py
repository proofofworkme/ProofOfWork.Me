...
COMMIT='6eb4a1e07131fc0b27d381dda52986f9d1bf3398'
OLD='07929dd3c6e422fa1955c6cbf07007330eca0519'
TREE='a77f1562fb5b42fcc8c7e45e109853e720742cfb'
RUNTIME='386a9abb411074c2f2a3aa5207a1a6ffd5e7c4901e08b3cd4591c4ee6fcb91f8'
LIVE='/opt/proofofwork-api'; STAGE=LIVE+'-stage-'+RELEASE
PRIOR=pathlib.Path('/data/proofofwork-audit17-cutover-'+RELEASE)
OUT=pathlib.Path(str(PRIOR)+'-attempt2')
...
proof=json.loads(pathlib.Path('/home/powadmin/audit17-shadow-core-6eb4a1e-attempt2/receipt.json').read_text());assert proof['ok'] is True
assert hashlib.sha256((PRIOR/'failure.json').read_bytes()).hexdigest()=='e428a8e1a3975d68e5121dbde49d0e04c43d91025d25d1118c51f8c79f4de2e1'
assert json.loads((PRIOR/'failure.json').read_text())['phase']=='stopping'
assert not (PRIOR/'exchange.json').exists() and not (PRIOR/'archive.json').exists()
prior=json.loads((PRIOR/'before.json').read_text())
assert attest(LIVE)==prior['oldAttestation'] and attest(STAGE)==prior['newAttestation']
...
save('before.json',{'at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'units':baseline,'oldAttestation':old_att,'newAttestation':new_att})
...
try:
    ...
    shadow=state('proofofwork-audit17-boost-shadow-attempt2.service')
    if shadow['LoadState']=='loaded':
        run(['systemctl','stop','proofofwork-audit17-boost-shadow-attempt2.service'])
    else:
    ...
    request=pathlib.Path('/var/tmp/proofofwork-deploy/proofofwork-node-release-6eb4a1e-20260919T161919Z.tgz')
