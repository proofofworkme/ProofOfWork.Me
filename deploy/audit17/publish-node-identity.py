...
os.umask(0o077)
RELEASE='85ce5651ad4b-20260919T170643Z'
COMMIT='85ce5651ad4b1403913371a21b42d9bec0941855'
OLD='005a4e582f5887c51865fbedc757d4597be9df4b'
TREE='86b9fd3a11bca89dbc6664b168ca3a911667042b'
RUNTIME='2e3ec1336a7b47f2c0e882ddd07bdc8fce0347184bbe23c8ee65951d618078a7'
...
proof=json.loads(pathlib.Path('/home/powadmin/audit17-shadow-core-85ce565/receipt.json').read_text());assert proof['ok'] is True
...
try:
    ...
    shadow=state('proofofwork-audit17-identity-shadow.service')
    if shadow['LoadState']=='loaded':
        run(['systemctl','stop','proofofwork-audit17-identity-shadow.service'])
    else:
    ...
    request=pathlib.Path('/var/tmp/proofofwork-deploy/proofofwork-node-release-85ce565-20260919T170643Z.tgz')
