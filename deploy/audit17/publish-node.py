...
OLD='8223f1dd338beaa71509332b511271be3e93f22a'
TREE='5ca993c7816b20c3b914e4421bdbdabf49673c5f'
RUNTIME='641de0f01428b19ae5d9baaa5ff66b59700c84570713808da929d0be751e19b3'
LIVE='/opt/proofofwork-api'; STAGE=LIVE+'-stage-'+RELEASE
OUT=pathlib.Path('/data/proofofwork-audit17-cutover-'+RELEASE+'-attempt3')
...
proof=json.loads(pathlib.Path('/home/powadmin/audit17-shadow-core-07929dd/receipt.json').read_text());assert proof['ok'] is True
...
try:
    ...
    shadow=state('proofofwork-audit17-shadow.service')
    if shadow['LoadState']=='loaded':
        run(['systemctl','stop','proofofwork-audit17-shadow.service'])
    else:
    ...
    assert run(['sudo','-n','-u','postgres','psql','-X','-qAt','-v','ON_ERROR_STOP=1','-d','proof_indexer','-c','SELECT count(*) FROM pg_stat_activity WHERE datname=current_database() AND pid<>pg_backend_pid()'])=='0', 'Database session remains'
    ...
    request=pathlib.Path('/var/tmp/proofofwork-deploy/proofofwork-node-release-07929dd-20260919T145727Z.tgz')
