...
- Exposes Credits and Bonds AMO tabs over confirmed credit creations, mints, transfers, holders, registries, active sale-ticket listings, settled credit sales, and POWB/INCB hard-price bond sale-ticket books with Inception and Infinity sub-tabs. The POWB/INCB hard-price declaration source is `server/bond-hard-price-declaration.mjs`; canonical declaration txid `ae7baca8bddc950af2d19e515e4df551cbb33e2ad61aa072f862700135164284` confirmed at height `963758`/index `1050` and activates at height `963759`. `npm run build:bond-hard-price-declaration` emits its exact `pwm1:m:` commitment and `npm run prepare:bond-hard-price-declaration` emits the authority-input record plus confirmed pins.
