...
The retry4 candidate was bound to release
`a4d2548f55b0-20260922T040224Z` (commit
`a4d2548f55b08c03a590a10e150f70c2f74ad758`, tree
`6faf073fa2ad9d017cd5a28a226bbbc86c755dbe`), with evidence preserved at
`/data/proofofwork-audit5-probe-a4d2548f55b0-20260922T040224Z-retry4/attempt`.
After 1,047 bounded HTTP/Core reads, retry4 stopped at
`WALLET_SOURCE_CHECKPOINT`. The wallet-token response exposed its indexed block
height but omitted `indexedThroughBlockHash`; the verifier incorrectly
required that unavailable field. The saved response height matched Core's
height. The next source revision accepts the matching height when that route
omits its hash, still rejects a mismatching hash when one is supplied, and
continues exact height/hash fencing on complete checkpointed routes. Retry4
and all earlier evidence remain immutable. No production switch is permitted
