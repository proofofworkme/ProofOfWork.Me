...
| PostgreSQL baseline (about 05:00 UTC) | proof_indexer measured 33,448,877,079 B. work_amo_block_transitions was 32,043,941,888 B; the recorded 8,725 live rows were a pg_stat estimate, with zero estimated dead rows. It accounted for about 95.8% of the database. ledger_snapshots was 830,275,584 B; events 156,213,248 B; transactions 106,561,536 B. |
...
| H8-05, H10-02, backups/database growth | Open. At 05:20 UTC, the database measured 33,508,563,991 B and the transition relation 32,103,456,768 B, including 32,094,683,136 B of TOAST. An exact metadata-only count found 8,738 rows over heights 959621–968358 with no height gaps. The 59.5 MB relation-size delta versus the 05:00-labeled sample is not enough to establish a steady rate. PostgreSQL backup storage was 92.49 GB at its earlier sample. The current dump restored successfully; storage redesign still requires isolated replay proof. |
...
- This continuation does not close the scheduled-backup timer gate, UI capacity monitoring, transition-table growth measurement, or the remaining read-latency/availability follow-ups listed above. At this 05:15 UTC checkpoint, the documentation PR was awaiting merge; the following entry records its later merge. No production deploy was required.

PR #63 later merged at 2026-09-24T05:18:15Z as `ce514230244ad577684c3ec24e7a35b37a6234f8`. All Node 20/22/24 repository-hygiene jobs passed; no production deploy was needed for the documentation-only correction.

## Database-growth checkpoint — 2026-09-24 05:24 UTC

A read-only Node VPS sample at 05:20:47 UTC measured the `proof_indexer` database at 33,508,563,991 B. The `proof_indexer.work_amo_block_transitions` relation measured 32,103,456,768 B: 5,914,624 B heap, 2,826,240 B indexes and 32,094,683,136 B TOAST. The earlier sample labeled 05:00 UTC measured 33,448,877,079 B and 32,043,941,888 B respectively, a measured difference of 59,686,912 B for the database and 59,514,880 B for the transition relation. The older 8,725 live-row value came from PostgreSQL statistics, not an exact count, so this does not establish an exact row-growth count or stable rate.

A separate metadata-only count at 05:22:38 UTC found exactly 8,738 distinct transition rows covering every height from 959621 through 968358. Dead rows were estimated at zero. The latest ten metadata rows each declared 2,522,140-byte opening and closing state payloads; some had zero events. Source review confirms that the immutable transition row stores a JSONB payload and the writer validates both opening and closing sufficient states. This follow-up did not read the stored JSONB values or compare the 05:20 transition tip with a fresh full-node tip.

The transition table’s size is a material growth risk, but the short size delta cannot safely be extrapolated. Keep H8-05/H10-02 open. Prototype any deduplicated or checkpointed representation on a fresh isolated restore, then verify every height, event commitment, exact balance and replay result before considering a versioned migration. Preserve the immutable live table and rollback until that proof passes. No production data, table, index, WAL, protocol record or backup was changed in this sample.
