...
- Indexed mailbox reads come from the proof-index address-mail projection when supported; confirmed self-sends must render in both Inbox and Sent. `pwm1:m:powb` Infinity Bonds and `pwm1:m:incb` Inception Bonds remain mailbox-visible while classifying as `infinity-bond` and `inception-bond` respectively in Log/Event History. Subject metadata from `pwm1:s` must stay separate from body text from `pwm1:m`; `mail_items.body_text` and UI memo rendering must not fall back to Log detail strings such as `Subject: ...`. Synthetic POWB/INCB mint credit belongs to each bond recipient address; self-sends credit the sender because the sender is also the recipient. Any attached credit is a separate canonical WORK transfer and must not alter bond-family supply. Inception may join a valid confirmed same-transaction WORK attachment into its frozen/live composite display, but the shared Growth/WORK ledger still counts that WORK movement only once.
