...
- Wallet labels proof balances spendable only after the connected address's complete ID and all credit/bond reservation lanes are ready. Loading or failed reservations remain explicit unavailable states. Bond summaries may render before the complete listing book finishes; AMO loads the selected bond reference on entry. Neither display readiness change relaxes fresh signing preflights.
...
- Indexed mailbox reads come from the proof-index address-mail projection when supported; confirmed self-sends must render in both Inbox and Sent. `pwm1:m:powb` Infinity Bonds and `pwm1:m:incb` Inception Bonds remain mailbox-visible while classifying as `infinity-bond` and `inception-bond` respectively in Log/Event History. Subject metadata from `pwm1:s` must stay separate from body text from `pwm1:m`; `mail_items.body_text` and UI memo rendering must not fall back to Log detail strings such as `Subject: ...`. Synthetic POWB/INCB credit belongs to each bond recipient address; self-sends credit the sender because the sender is also the recipient. POWB stays one-for-one with recipient proof payments. INCB issuance equals direct bond proofs plus valid recipient-matched same-transaction WORK valued by the send-time oracle: the last confirmed green canonical live WORK summary at H-1, hash-bound to the exact previous block, at one INCB per whole proof. Every transaction in the bond block is excluded. The same oracle fixes the attached transfer's frozen WORK movement value. Confirmation fixes the resulting INCB balance, supply, and attached WORK proof value. Inception network value equals fixed cumulative issuance value plus confirmed INCB sale volume, transfer fees, and marketplace mutation fees; current or later WORK value never reprices INCB. The attached WORK remains a separate canonical transfer that the shared Growth/WORK ledger counts only once.
...
AMO actions should stay outside the mailbox folders. Keep ID, credit, and bond trading in the Computer AMO workspace and `amo.proofofwork.me`, while mail organization remains focused on messages, files, contacts, drafts, and local folders. The legacy Marketplace hostname points to AMO. The AMO workspace is tabbed by asset class: IDs, Credits, and Bonds use sale-ticket settlement; the Bonds tab separates Inception and Infinity listings, while Wallet stays the place to transfer or list owned credit and bond balances.
