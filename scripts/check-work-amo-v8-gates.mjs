...
function broadcastAdmissionFixture() {
  ...
  const { assertWorkMarketplaceBroadcastAllowed } = isolatedFunctions(
    ...
    ["assertWorkMarketplaceBroadcastAllowed"],
    {
      ...
      WORK_AMO_V8_DECLARATION_PINS_REQUESTED: true,
      WORK_AMO_V8_DECLARATION_PIN_STATE: "configured",
...

{
  ...
  const { workAmoV8ReplayPrecisionOptions } = isolatedFunctions(
    ...
    ["workAmoV8ReplayPrecisionOptions"],
    {
      ...
      proofIndexWorkAmoV8ActivationLatch: async () => ({
        markerReady: true,
        pins: {
  ...
  const current = await workAmoV8ReplayPrecisionOptions(
    "livenet",
    102,
  );
  assert.equal(current.workAmoV8ActivationHeight, 101);
  assert.equal(
    current.workAmountStorageModel,
    "work-subatoms-v2",
  );
  const historical = await workAmoV8ReplayPrecisionOptions(
    "livenet",
    100,
  );
  assert.equal(historical.workAmoV8ActivationHeight, 101);
  assert.equal(
...

{
  ...
  const exact = await workAmoV8ReplayPrecisionOptions(
    "livenet",
    500,
    checkpointHash,
  );
...

{
  ...
  assert.equal(
    preactivation.workAmoV8,
    null,
    "next-block replay must remain live before any V8 declaration latch exists",
  );
  assert.equal(
  ...
  assert.equal(beforeActivation.workAmoV8, null);
  await assert.rejects(
  ...
  assert.equal(activated.workAmoV8?.activationHeight, 501);
  assert.equal(
    activated.workAmoV8?.amountStorageModel,
    "work-subatoms-v2",
  );
}

const declarationCommitment = Object.freeze({
  protocolRecord: "pwm1:m:work-amo-v8-declaration",
...
function apiDeclarationDiscoveryFixture(configured, candidates) {
  ...
  const discovery = isolatedFunctions(
    apiSource,
    [
      "workAmoV8DeclarationCandidatesFromCanonicalTransactions",
    ...
    ],
    {
      ...
      configuredWorkAmoV8Declaration: () => configured,
      ...
      proofIndexWorkAmoV8DeclarationCandidates: async (
        _network,
        options,
      ) => {
        indexReads += 1;
...
function indexDeclarationDiscoveryFixture(configured) {
  return isolatedFunctions(
    backfillSource,
    [
      "exactWorkAmoV8DeclarationPins",
      "discoverIndexedWorkAmoV8DeclarationPins",
    ],
    {
      NETWORK: "livenet",
      ...
      configuredWorkAmoV8DeclarationPins: () => configured,
...

{
  ...
  assert.match(
    capturedSql,
    /SELECT count\(\*\)[\s\S]*proof_indexer\.op_returns exact_carrier[\s\S]*\) = 1/iu,
    "invalid duplicate-carrier declarations must be filtered before canonical ordering",
  );
  assert.match(
    capturedSql,
    /SELECT count\(\*\)[\s\S]*proof_indexer\.tx_outputs exact_registry[\s\S]*\) = 1/iu,
    "invalid duplicate-payment declarations must be filtered before canonical ordering",
  );
  ...
  await assert.rejects(
    ...
    /indexed AMO V8 declaration selection diverged/iu,
    "a database result that violates the valid-candidate SQL must fail closed",
  );
}
