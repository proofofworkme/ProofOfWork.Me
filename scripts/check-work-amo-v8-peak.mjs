...
function transactionId(index) {
  return createHash("sha256")
    .update(`work-amo-v8-peak:${index}`, "utf8")
    .digest("hex");
}
...
function listingAuthorization(transactionIndex) {
  return {
    ...
    expiresAt: "",
    network: "livenet",
    nonce: `work-amo-v8-peak-${transactionIndex}`,
    ...
    unitFaceProofs: WORK_AMO_V8_ALLOWED_FACE_PROOFS[
      transactionIndex % WORK_AMO_V8_ALLOWED_FACE_PROOFS.length
    ],
...
function replay(inputRecords) {
  return replayWorkAmoV8CanonicalBlock({
    activationHeight,
    ...
    evaluateRecord: ({
      ...
      if (entry.eventKind === "listing") {
        ...
        const derived = deriveWorkAmoV8FrozenTerms(
          entry.authorization,
          {
            activationHeight,
        ...
        assert.equal(
          ...
          true,
          derived.reasonCode ??
            "peak V8 listing must derive frozen terms",
        );
        const beforeFormula = calculateWorkAmoV8UnitTerms({
          networkValueBeforeQ8,
          unitFaceProofs:
            entry.authorization.unitFaceProofs,
        });
        const afterFormula = calculateWorkAmoV8UnitTerms({
          networkValueBeforeQ8:
