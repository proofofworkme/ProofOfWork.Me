...
import ast,copy,datetime,io,pathlib
source=pathlib.Path('deploy/audit17/publish-node.py').read_text(); tree=ast.parse(source)
...
for fault in ('none','shadow-present','database-session','readiness','exchange-ambiguous'):
    states={u:{'LoadState':'loaded','ActiveState':'active',**({} if u.endswith('.socket') else {'MainPID':'1'})} for u in apps+keep+timers}
    baseline=copy.deepcopy(states); old=['old'];new=['new'];live=[False]; saved={}; calls=[]
    class Paths:Path=FakePath
    def state(u):
        if u=='proofofwork-audit17-shadow.service' and u not in states:
            return {'LoadState':'not-found','ActiveState':'inactive'}
        return states[u].copy()
    if fault=='shadow-present':states['proofofwork-audit17-shadow.service']={'LoadState':'loaded','ActiveState':'active','MainPID':'7'}
    def run(args,*rest):
        calls.append(args)
        if args[0]=='systemctl':
            for u in args[2:]:
            ...
            return ''
        if args[0]=='sudo':return '1' if fault=='database-session' else '0'
    ...
    if fault=='exchange-ambiguous':
        ...
        assert live[0]==(fault in ('none','shadow-present'))
