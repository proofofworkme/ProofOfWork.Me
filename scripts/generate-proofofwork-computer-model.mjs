...
// Historical source: preserve its generated bytes and write only the archive.
const HISTORICAL_OUTPUT_DIR = "output/historical/2026-05-13";
mkdirSync(HISTORICAL_OUTPUT_DIR, { recursive: true });
const OUTPUT = `${HISTORICAL_OUTPUT_DIR}/proofofwork-computer-agent-adoption-model.md`;
const GROWTH_JSON_OUTPUT = `${HISTORICAL_OUTPUT_DIR}/proofofwork-computer-growth-model.json`;
...
function writeVisuals(rows) {
  const visuals = [
    [`${HISTORICAL_OUTPUT_DIR}/proofofwork-computer-model-compounding.svg`, renderCompoundingVisual()],
    [`${HISTORICAL_OUTPUT_DIR}/proofofwork-computer-model-dollar-growth.svg`, renderDollarGrowthVisual(rows)],
    [`${HISTORICAL_OUTPUT_DIR}/proofofwork-computer-model-product-split.svg`, renderProductSplitVisual(rows)],
    [`${HISTORICAL_OUTPUT_DIR}/proofofwork-computer-model-blockspace.svg`, renderBlockspaceVisual(rows)],
    [`${HISTORICAL_OUTPUT_DIR}/proofofwork-computer-model-volatility.svg`, renderVolatilityVisual(rows)],
  ];
