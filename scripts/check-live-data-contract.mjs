...
expectAll("wallet scoped WORK reads do not publish orphan Q16 price aliases", server, [
  ...
  /async function walletScopedTokenPayload\([\s\S]*const withWalletAuthority = \(payload\) => \{[\s\S]*walletScopedWorkPayloadWithQ16Supply\([\s\S]*workQ16PayloadWithoutOrphanPriceAliases\(normalizedPayload,\s*scope\)[\s\S]*walletScopedPayloadUsesAuthoritativeOverlay\(walletPayload\)/,
