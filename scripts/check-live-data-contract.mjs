...
expectAll("hot worker summary publication is canonical, conservative, and health-gated", proofIndexerBackfill + proofIndexReader + server, [
  ...
  /payloadWithCanonicalWorkLifecyclePositions\([\s\S]*closedLifecycleListingKeys[\s\S]*tokenListingsWithoutClosedEvents\([\s\S]*payload\.listings[\s\S]*closedListings[\s\S]*closedLifecycleListingKeys\.has[\s\S]*positionedListings/,
