...
expectAll("wallet scoped WORK reads do not publish orphan Q16 price aliases", server, [
  ...
  /async function walletScopedTokenSummaryPayload\([\s\S]*?if \(!scope \|\| scope === WORK_TOKEN_ID \|\| requireCurrent\) \{\s*return compactTokenSummaryPayload\(\s*await walletScopedTokenPayload\([\s\S]*?allowLastGood: options\.allowLastGood === true[\s\S]*?requireCurrent/,
