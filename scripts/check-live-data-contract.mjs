...
expectAll("marketplace summary and tabs keep confirmed sealed inventory canonical", server + app, [
  /const MARKETPLACE_SUMMARY_FRESH_WAIT_MS = Number\([\s\S]*Math\.max\(WORK_FLOOR_FRESH_WAIT_MS,\s*30_000\)/,
  ...
  /if \(marketplaceSummaryHasIndexedMarketOverlay\(fallback\)\) \{[\s\S]*refreshPayloadCacheInBackground/,
