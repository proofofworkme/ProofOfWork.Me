...
expectAll("wallet scoped token reads keep confirmed lifecycle history", server, [
  ...
  /async function walletScopedTokenPayload\([\s\S]*currentCanonicalWorkTokenSummaryPayloadForFreshRead\([\s\S]*currentProofIndexTokenPayloadForRead\([\s\S]*Fresh wallet credit state is temporarily unavailable/,
