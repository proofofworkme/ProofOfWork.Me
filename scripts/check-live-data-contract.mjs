...
expectAll("server fresh token state reads fall back to valid cached snapshots", server, [
  ...
  /async function currentExactTipTokenPayloadForRead\([\s\S]*canonicalGate = null[\s\S]*EXACT_TIP_TOKEN_CACHE\.get\(cacheKey\)[\s\S]*validatedUntil[\s\S]*canonicalGate\.atTip !== true[\s\S]*canonicalHash !== indexedThroughBlockHash[\s\S]*retainedExactTipTokenPayloadForRead\(payload, canonicalGate, cached\)[\s\S]*healthNodeTipHeight\(\)[\s\S]*!Number\.isSafeInteger\(tipHeight\)[\s\S]*return null;[\s\S]*indexedThroughBlock === tipHeight[\s\S]*return payload;[\s\S]*EXACT_TIP_TOKEN_CACHE\.delete\(cacheKey\)/,
  ...
  /if \(indexedPayload\) \{[\s\S]*const responsePayload = await withWorkMarketplaceV4Metadata\([\s\S]*indexedPayload,[\s\S]*network,[\s\S]*\);[\s\S]*cacheTokenPayload\(network,\s*tokenScope,\s*responsePayload,\s*\{[\s\S]*exactTipValidated:\s*true[\s\S]*\}\)[\s\S]*jsonResponse\([\s\S]*responsePayload/,
