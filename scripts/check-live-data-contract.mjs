...
expectAll("server fresh token reads admit only exact bounded state", server, [
  ...
  /function freshWorkTokenIndexedMissDisposition\([\s\S]*!final \|\| final !== admitted \? "changed" : "catching-up"/,
  ...
  /const indexedPayloadExact = tokenPayloadMatchesCanonicalGate\([\s\S]*network === "livenet" &&[\s\S]*freshRead &&[\s\S]*workTokenAuthorityRequired[\s\S]*canonicalPublicReadGate\(network, \{[\s\S]*force: true[\s\S]*freshWorkTokenIndexedMissDisposition\([\s\S]*tokenReadCanonicalIdentity,[\s\S]*canonicalReadGateIdentity\(finalGate, tokenScope\)[\s\S]*Fresh credit state changed while it was being verified[\s\S]*Fresh credit state is still catching up[\s\S]*tokenPayloadForRead\(/,
