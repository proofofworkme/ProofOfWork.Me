...
expectAll("Infinity Bond POWB recipient-credit market is wired", server + app + routeRegistry, [
  ...
  /async function infinitySummaryPayload\(network,\s*fresh\s*=\s*false\)[\s\S]*summaryCanonicalLedgerPayload\(network,\s*true\)[\s\S]*existingCanonicalLedgerPayload\(network\)[\s\S]*infinitySummaryFromCanonicalLedger\(ledger,\s*network,\s*fresh\)/,
