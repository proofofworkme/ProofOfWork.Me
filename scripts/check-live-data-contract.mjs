...
expectAll("marketplace summary and tabs keep confirmed sealed inventory canonical", server + app, [
  ...
  /async function marketplaceSummaryPayloadWithIndexedMarketOverlay\([\s\S]*indexedTokenMarketSummaryOverlay\([\s\S]*tokenPayloadWithCurrentWorkActiveListingPolicy\([\s\S]*compactTokenSummaryPayload\(currentTokenState\)/,
  ...
  /const indexedAt = newerIso\(\s*ledger\.generatedAt,\s*currentTokenState\?\.indexedAt,\s*\)/,
