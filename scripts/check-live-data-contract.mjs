...
const bondHardPriceDeclarationBuilder = readFileSync(
...
expectAll("server compact token summaries preserve existing row supply metrics", server, [
  ...
  /function mergedTokenSummaryMetric\([\s\S]*?\{ workAmountStorageModel = "" \} = \{\},[\s\S]*?preserveExisting && tokenValue !== undefined/,
  /const preserveExistingTokenMetrics =\s*payload\.summaryOnly === true \|\| walletScopedSummary/,
  /confirmedSupply:\s*mergedTokenSummaryMetric\(\s*token,\s*summary,\s*"confirmedSupply",\s*preserveExistingTokenMetrics,\s*summaryMetricOptions,\s*\)/,
  /pendingSupply:\s*mergedTokenSummaryMetric\(\s*token,\s*summary,\s*"pendingSupply",\s*preserveExistingTokenMetrics,\s*summaryMetricOptions,\s*\)/,
  /next\.confirmedSupplySubatoms = confirmedSupplySubatoms/,
  /next\.pendingSupplySubatoms = pendingSupplySubatoms/,
...
expectAll("wallet scoped token reads keep confirmed lifecycle history", server, [
  ...
  /async function walletScopedTokenPayload\([\s\S]*?return await walletScopedTokenSingleFlight\(walletCacheKey,[\s\S]*?cachedWalletScopedTokenPayload\(walletCacheKey\)/,
...
expectAll("wallet scoped WORK reads do not publish orphan Q16 price aliases", server, [
  ...
  /function walletScopedWorkSupplySubatoms\([\s\S]*?canonicalWorkSubatomsText\([\s\S]*?parseWorkAmountToSubatoms\([\s\S]*?return "0"/,
  /function walletScopedWorkTokenWithQ16Supply\([\s\S]*?canonicalWorkTokenDefinition\(\s*network,\s*WORK_SUBATOM_PROJECTION_MODEL[\s\S]*?confirmedSupplySubatoms[\s\S]*?pendingSupplySubatoms[\s\S]*?precisionModel: WORK_PRECISION_V2_MODEL/,
  /function walletScopedWorkPayloadWithQ16Supply\([\s\S]*?singleWorkEnvelope[\s\S]*?amountStorageModel: WORK_SUBATOM_PROJECTION_MODEL[\s\S]*?confirmedSupplySubatoms: workToken\.confirmedSupplySubatoms[\s\S]*?pendingSupplySubatoms: workToken\.pendingSupplySubatoms/,
  /function walletScopedTokenPayloadFromOverlay\([\s\S]*?return walletScopedWorkPayloadWithQ16Supply\(\{[\s\S]*?confirmedSupply: 0[\s\S]*?pendingSupply: 0[\s\S]*?\}, network, tokenScope\)/,
  ...
  /async function walletScopedTokenPayload\([\s\S]*?const withWalletAuthority = \(payload\) => \{[\s\S]*?walletScopedWorkPayloadWithQ16Supply\([\s\S]*?workQ16PayloadWithoutOrphanPriceAliases\(\s*normalizedPayload,\s*scope,?\s*\)[\s\S]*?walletScopedPayloadUsesAuthoritativeOverlay\(walletPayload\)/,
  /async function walletScopedTokenSummaryPayload\([\s\S]*?const checkedSummary = \(payload\) => \{\s*const summary = compactTokenSummaryPayload\(payload, scope\);\s*assertCanonicalCreditAggregateResponse\(summary, network\);[\s\S]*?if \(!scope \|\| scope === WORK_TOKEN_ID \|\| requireCurrent\) \{\s*return checkedSummary\(\s*await walletScopedTokenPayload\([\s\S]*?allowLastGood: options\.allowLastGood === true[\s\S]*?requireCurrent/,
...
expectAll("wallet balance reads require an exact hashed checkpoint or bounded canonical last-good gate", server, [
  ...
  /function walletTokenOverlayMatchesCanonicalIndexedGate\(overlay,\s*gate\)[\s\S]*?walletTokenOverlayHasExactCheckpoint\(overlay\)[\s\S]*?Number\(gate\?\.indexedThroughBlock\) === overlayHeight[\s\S]*?canonicalHash === overlayHash[\s\S]*?storedHash === overlayHash/,
...
expectAll("marketplace summary and tabs keep confirmed sealed inventory canonical", server + app, [
  ...
  /async function marketplaceSummaryPayloadWithIndexedMarketOverlay\([\s\S]*?indexedTokenMarketSummaryOverlay\([\s\S]*?tokenPayloadWithCurrentWorkActiveListingPolicy\([\s\S]*?tokenStateWithCanonicalWorkListingBookForMarketplaceSummary\([\s\S]*?compactTokenSummaryPayload\(canonicalTokenState\)/,
  ...
  /function tokenSummaryMarketPreviewRecord\([\s\S]*?TOKEN_SUMMARY_MARKET_PREVIEW_KEYS[\s\S]*?includeNestedListings[\s\S]*?"closedListing",\s*"listing"[\s\S]*?TOKEN_SUMMARY_NESTED_LISTING_PREVIEW_KEYS/,
  /const compactMarketRecords = options\.compactMarketRecords === true[\s\S]*?tokenSummaryMarketPreviewRecord\(record\)[\s\S]*?includeNestedListings:\s*true[\s\S]*?\.map\(compactClosedMarketRecord\)/,
  ...
  /const indexedAt = newerIso\(\s*ledger\.generatedAt,\s*canonicalTokenState\?\.indexedAt,\s*\)/,
...
expectAll(
  ...
  [
    ...
    /function workAmoReplayReadinessRequest\([\s\S]*?Object\.freeze\(\{ \.\.\.options \}\)[\s\S]*?requestOptions\.force === true[\s\S]*?requestOptions\.singleFlightBypass === true/,
    /function exactCheckpointSingleFlight\([\s\S]*?inFlight\.set\(key, pending\)[\s\S]*?finally[\s\S]*?inFlight\.delete\(key\)/,
    /function deepFreezeReadinessSnapshot\([\s\S]*?Object\.values\(value\)[\s\S]*?Object\.freeze\(value\)/,
...
expectAll("summary proof-index reads reject stale snapshot ids", server, [
  ...
  /url\.pathname === "\/api\/v1\/work-floor"[\s\S]*?workFloorResponsePayload\(network,\s*freshRead\)/,
  /async function workFloorResponsePayload\(network,\s*fresh\s*=\s*false\)[\s\S]*?deduplicatedSummaryRead\([\s\S]*?withWorkMarketplaceV4Metadata\([\s\S]*?summaryPayloadWithCanonicalProvenance\([\s\S]*?cachedWorkFloorPayload\(network,\s*fresh\),\s*network,\s*fresh,\s*"work-floor"/,
...
expectAll("historical AMO V5 stays fail-closed", proofIndexDeploy, [
...
expectAll("historical AMO V5 derives the pinned declaration height only from a complete Core position proof", server, [
...
expectAll("historical AMO V5 uses exact position-derived immutable unit terms", workAmoV5Core, [
...
expectAll("historical AMO V5 migration preserves relic and invalid-history distinctions", workAmoV5Migration, [
...
expectAll(
  "AMO V8 worker treats any requested pin as fail-closed and requires exact D+1 configuration",
  proofIndexerWorker,
  [
    /export function workerWorkAmoV8DeclarationConfig/,
...
expectAll(
  "AMO V8 API, backfill, and precision migration reject noncanonical pin aliases consistently",
  `${server}\n${proofIndexerBackfill}\n${workAmoV8Migration}`,
  [
    /function canonicalWorkAmoV8ConfiguredInteger[\s\S]*?const raw = String\(value \?\? ""\);[\s\S]*?\/\^\(\?:0\|\[1-9\]\[0-9\]\*\)\$\//,
    /function canonicalWorkAmoV8ConfiguredHash[\s\S]*?const raw = String\(value \?\? ""\);[\s\S]*?\/\^\[0-9a-f\]\{64\}\$\//,
    /WORK_AMO_V8_WRITES_SOURCE[\s\S]*?WORK_AMO_V8_WRITES_SOURCE !== WORK_AMO_V8_WRITES_RAW/,
    /WORK_AMO_V8_DECLARATION_PINS_REQUESTED[\s\S]*?\.some\(\(value\) => String\(value \?\? ""\)\.length > 0\)/,
...
expectAll(
  "AMO V8 worker binds Q16 replay to stable Core, exact DB tip hashes, chain continuity, snapshot, and relational parity",
  proofIndexerWorker,
  [
...
expectAll(
  ...
  [
    ...
    /runCanonicalBeforePending\([\s\S]*?runBackfillPhase\(backfillPhases\[0\]\)[\s\S]*?publishCanonicalSummaryAtConfirmedCheckpoint\(\)[\s\S]*?pendingStatus = await refreshPendingStatusesSafely\(\);[\s\S]*?runBackfillPhase\(backfillPhases\[1\]\)/,
...
expectAll(
  "AMO V8 backfill owns one durable pending witness and marks it ready only after exact Core and DB recomputation",
  proofIndexerBackfill,
  [
...
expectAll(
  "WORK V8 keeps the historical Q8 scale immutable and introduces explicit Q16 subatoms",
  `${workUnits}\n${workAmoV8Core}`,
  [
    /WORK_DECIMALS = 8/,
    ...
    /WORK_AMO_V8_SUBATOMS_PER_WORK =\s*WORK_SUBATOM_UNIT_SCALE/,
  ],
);
expectAll(
...
expectAll(
  "AMO V8 derives the singleton Q16 face and excludes pre-V8 relics from active settlement",
  workAmoV8Core,
  [
    /WORK_AMO_V8_ALLOWED_FACE_PROOFS = Object\.freeze\(\[\s*25_000,\s*\]\)/,
    /export function workAmoV8UnitTerms\([\s\S]*?unitAmountSubatoms = workAmoFloorDiv[\s\S]*?unitMinimumPriceSats = workAmoCeilDiv/,
    /function canonicalWorkAmoV8TokenStateListing[\s\S]*?version === WORK_AMO_V8_AUTH_VERSION[\s\S]*?return null/,
    /validateWorkAmoV8SealOrBuyTerms[\s\S]*?listingVersion !== WORK_AMO_V8_AUTH_VERSION[\s\S]*?work-amo-v8-relic-listing-nonsettleable/,
  ],
);
expectAll(
  ...
  `${workAmoV8Core}\n${server}`,
  [
    /export function workAmoV8TransferEraDecision[\s\S]*?v8Required = height >= activation[\s\S]*?nativeV8 !== v8Required[\s\S]*?work-amo-v8-send3-required[\s\S]*?work-amo-v8-send3-before-activation/,
    /TOKEN_SEND_SUBATOMS_ACTION = WORK_AMO_V8_TRANSFER_VERSION/,
    ...
    /workAmoV8TransferEraDecision\([\s\S]*?activationHeight:[\s\S]*?blockHeight,[\s\S]*?confirmed,[\s\S]*?projectionModel:[\s\S]*?transferVersion: parsedTransferVersion/,
  ],
);
expectAll(
...
expectAll(
  ...
  [
    ...
    /CREATE TABLE IF NOT EXISTS proof_indexer\.work_amo_v8_listing_terms/,
...
expectAll(
  ...
  [
    ...
    /valueSignalRule=mail recipients, self-send outputs, attached WORK, additional payments, and miner fee can signal value/,
    ...
    /workSeparationRule=POWB and INCB are not governed WORK units/,
    /settlementRule=a confirmed bond listing may be sealed, bought, or delisted only against its frozen signed terms/,
...
expectAll(
  ...
  [
    ...
    /complete pinned 46-mint historical INCB issuance set/,
    /CANONICAL_INCB_ISSUANCE_REPAIR_EXPECTATION_ROWS/,
    /acceptedMints:\s*46/,
    /parentBondEvents:\s*47/,
    /directProofIssuanceUnits:\s*"27386"/,
    /attachedWorkIssuanceUnits:\s*"224847713398420540"/,
    /confirmedSupply:\s*"224847713398447926"/,
    /networkValueQ8:\s*"22484771339844794793582060"/,
    /issuanceDustQ8:\s*"2193582060"/,
    ...
    /\["dd743fb69c519200cc190627219ba34ca2e63e6893e600b73e9aee8d4dac8fa4","000000000000000000016ea78b0d57a7979de3542518c8690a1e5a808e691cc5",957950,382,"1BPVvi1GK4QkfqFMU4jHGjsQjyGwjJJJ7x","546",0,"3644060","","1421798915","1421799461","546"/,
    /\["a22d5e3c5f9325360f23d79c277ce3fee5f27a0d4d180a60054caaaf1312884b","00000000000000000000296088f006957c756fa7a1d6f9da3af2a647d7a36996",959004,1781/,
    ...
    /function canonicalIncbHistoricalMintAggregate\(/,
    /function assertCanonicalIncbHistoricalBaselineAggregate\(/,
    /async function verifiedCanonicalIncbHistoricalBaselineAggregate\(/,
    /verifiedCanonicalIncbHistoricalBaselineAggregate\(client\)/,
    /has no pinned historical oracle/,
    ...
    /kind = 'token-event-invalid'[\s\S]*?attemptedKind[\s\S]*?'token-mint'[\s\S]*?INCB_MISSING_ATOMIC_METADATA_REPAIR_REASON/,
    /DELETE FROM proof_indexer\.events[\s\S]*?kind = 'token-event-invalid'[\s\S]*?lower\(COALESCE\(payload->>'tokenId', ''\)\) = \$4/,
    /const upsert = await upsertEvent\([\s\S]*?sourceLabelForProtocolItem\(integrityItem\)[\s\S]*?integrityItem/,
...
expect(
  "INCB repair must not use stored confirmed mint rows as an oracle",
  !/canonicalIncbStoredConfirmedMint|repair-incb-issuance-stored-mint/.test(
    proofIndexerBackfill,
  ),
);
expect(
...
expect(
  "package exposes AMO V8 declaration, protocol, peak-load, and precision checks",
  /"build:work-amo-v8-declaration":\s*"node scripts\/build-work-amo-v8-declaration\.mjs"/.test(
    packageJson,
  ) &&
    /"check:work-amo-v8":\s*"node scripts\/check-work-amo-v8\.mjs && node scripts\/check-work-amo-v8-gates\.mjs"/.test(
      packageJson,
    ) &&
    /"check:work-amo-v8-peak":\s*"node --expose-gc scripts\/check-work-amo-v8-peak\.mjs"/.test(
      packageJson,
    ) &&
...
expect(
  "AMO V6 API readiness requires the dedicated V6 migration proof",
  /proofIndexWorkAmoV6MigrationReadiness/.test(server) &&
    /workAmoV6Metadata[\s\S]*?proofIndexWorkAmoV6MigrationReadiness/.test(
      server,
    ) &&
    /Number\.isSafeInteger\(WORK_AMO_V6_DECLARATION_RECORD_ORDINAL\) &&\s*WORK_AMO_V6_DECLARATION_RECORD_ORDINAL === 0/.test(
      server,
    ) &&
    /function workAmoV6ReplayInputsForBlock[\s\S]*?proofIndexWorkAmoV6MigrationReadiness\([\s\S]*?migrationReadiness\?\.ready !== true[\s\S]*?migrationReadiness\?\.active !== true[\s\S]*?migrationReadiness\?\.canonical !== true[\s\S]*?migrationReadiness\?\.confirmed !== true[\s\S]*?migrationReadiness\?\.evidenceComplete !== true/.test(
      server,
    ),
);
