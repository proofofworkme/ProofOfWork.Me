...
expectAll("hot worker summary publication is canonical, conservative, and health-gated", proofIndexerBackfill + proofIndexReader + server, [
  ...
  /async function tokenStatePayloadAtCanonicalCheckpoint\([\s\S]*checkpoint_block\.canonical = true[\s\S]*token_event\.protocol = 'pwt1'[\s\S]*token_event\.kind LIKE 'token-%'[\s\S]*token_event\.status = 'confirmed'[\s\S]*indexedThroughBlock: checkpointHeight[\s\S]*indexedThroughBlockHash: normalizedCheckpointHash/,
  ...
  /proofIndexLogHistoryPayload\([\s\S]*currentRelational[\s\S]*proofIndexCanonicalActivityPayload\(network, \{[\s\S]*snapshotId: pagination\.snapshotId[\s\S]*snapshotTotalCount/,
