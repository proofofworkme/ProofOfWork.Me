...
expectAll("WORK Growth Log and token views use the same ledger", server, [
  ...
  /async function cachedWorkFloorPayload\(network,\s*fresh\s*=\s*false\)[\s\S]*summaryCanonicalLedgerPayload\(network,\s*fresh\)[\s\S]*workFloorWithCurrentBtcUsd\(ledger\.workFloor,\s*network,\s*fresh\)[\s\S]*Current WORK floor ledger is unavailable/,
  /async function growthSummaryPayload\(network,\s*fresh\s*=\s*false\)[\s\S]*summaryCanonicalLedgerPayload\(network,\s*fresh\)[\s\S]*growthSummaryWithCurrentBtcUsd\(ledger\.growthSummary,\s*network,\s*fresh\)[\s\S]*Current Growth summary ledger is unavailable/,
...
expectAll("WORK floor USD uses live price metadata", server, [
  ...
  /url\.pathname === "\/api\/v1\/work-summary"[\s\S]*await workSummaryWithCurrentBtcUsd\(indexedPayload,\s*network,\s*false\)/,
