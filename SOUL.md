...
- WORK and credit listing views must preserve sale-ticket seal metadata when pending listings promote to confirmed state. Cache/regression guards should reject internally inconsistent payloads that drop a seal within the same authenticated canonical snapshot, while a newer verified full snapshot may remove it after a reorganization or repaired projection.
...
- Wallet, Credit, WORK, and AMO refreshes may preserve bounded public-only local lifecycle evidence for credit definitions, mints, transfers, listings, seals, closures, and sales while the indexer catches up. Confirmed chain/API state wins once every expected row is indexed; local confirmed-indexing or unknown records never become canonical balances, supply, volume, or action authority.
- Evidence-backed replaced, dropped, and failed WORK/credit transactions must leave live economic projections after lifecycle checks. They may remain diagnosable in bounded local history, but they must not keep stale transfers, listings, seals, or buys visible as live pending state, and absence alone must not manufacture a terminal label.
...
- Process availability and canonical readiness are different truths. `/health/live` answers only whether the API process can respond; `/health` stays fail closed until the node, index, worker, and authenticated canonical snapshot agree.
- A bounded or incomplete mempool pass degrades pending visibility; it must not crash-loop the worker or take confirmed canonical reads offline. Confirmed replay continues to advance, while pending writes remain fail closed until the next complete witness.
- A canonical snapshot is allowed to shrink. Pending removal, replacement, a chain reorganization, or a repaired projection can lower counts, supply, activity, or value. UI code must accept a newer verified full snapshot, including an empty one, and retain prior data only when the current read actually fails.
- Summary-only projections are partial by contract. They may update summary surfaces, but they must never erase, relabel, or silently masquerade as complete ID, credit, bond, marketplace, or Log collections.
- Public read states are explicit: loading, unavailable, verified last-good, ready, and wallet-disconnected are not zero. A zero count is displayed as zero only after a successful canonical read proves it.
- Transaction lifecycle labels require evidence. Current full-node presence wins for confirmed or pending. `unknown` means the current node cannot prove presence or a durable terminal state; `dropped` or `replaced` requires durable prior observation plus verified removal/conflict evidence; `failed` is a known rejection, not a synonym for absence. A later current observation can revive a stale terminal projection.
- RUSH protocol history remains inspectable as canonical history, but public RUSH query routing, reads, and mint entrypoints are staged and default-off until explicitly enabled.
- Expensive public reads must be bounded before they touch Core or PostgreSQL: validate query size and pagination, rate-limit and admit first, coalesce identical work, cap concurrency/queues/cache entries and bytes, and preserve immutable last-good cache values when a replacement is refused.
