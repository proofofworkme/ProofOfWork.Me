...
- `pwm1:m:incb` is the canonical Inception Bond memo. It must normalize to `inception-bond` and mint only INCB to the confirmed recipient. POWB and INCB definitions/mints are synthetic projections over their matching bond payments, not generic credit creates or proof-value events. A confirmed same-transaction canonical WORK attachment remains its own `pwt1:send` movement lane, but the Inception summary joins it into frozen confirmation-time and live current-floor composite bond value. INCB supply stays equal to confirmed bond proofs, and the shared Growth/WORK ledger counts the underlying WORK movement only once.
