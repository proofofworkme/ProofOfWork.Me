...
- WORK permanent floor formula: `work_floor_proofs = confirmed_network_value_proofs / 21,000,000 WORK`.
...
For USD in public posts, use the live BTC/USD-backed fields from the fresh API response: `actualValue.totalUsd`, `btcUsd`, `btcUsdIndexedAt`, and `usdSource`, or recompute from `/api/v1/prices/btc-usd`. Do not use `modelTotalUsd` for current tweet copy; it is the Growth model USD projection retained for comparison.

Daily tweet set:

- WORK mint-to-floor update. Report mint price, current confirmed floor proofs per WORK, floor multiple from mint, confirmed ProofOfWork Computer network value, and live USD if useful. Formula: `floor_proofs_per_work = floor.networkValueSats / 21000000`; mint multiple is `floor_proofs_per_work / 1`; live USD is `floor.actualValue.totalUsd` or `floor.networkValueSats / 100000000 * floor.btcUsd`.
